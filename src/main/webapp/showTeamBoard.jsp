...
<% String title = request.getParameter("title"); 
	...
	try {
		...
		pstmt = conn.prepareStatement("select * from baseball_db.team_board where text = ? and id = ?");
	...
	try {
		...
		pstmt = conn.prepareStatement("UPDATE baseball_db.team_board SET hit = ? WHERE id = ? AND (text = ? and team_name = ?)");
		pstmt.setInt(1, hit);
		pstmt.setString(2, writer);
									    ...
									    <div class="container1">
									        ...
									        try{
									    		...
									    		if(rs.next()){
									    			...
									    			<form method="post" action="updateTeamBoard.jsp?title=<%=title%>">
										        			<div class="post-info">
													        ...
													        <div class="post-content">
													           ...
													           <button type="button" onclick="location.href='teamBoard.jsp'">목록</button>
															         <button type="submit">수정</button>
															         <button type="button" onclick="location.href='deleteTeamBoard.jsp?title=<%=title%>&id=<%=id%>'">삭제</button>
									        ...
									        <div class="comment-section">
								                    ...
								                    <div>
								                    	<form id="commentForm" method="post" action="addTeamReple.jsp">
								                	...
								                	<% try{
											    		...
											    		Class.forName("com.mysql.cj.jdbc.Driver");
											    		conn = DriverManager.getConnection(dbURL, dbID, dbPassword);
											    		pstmt = conn.prepareStatement("select * from baseball_db.team_board_comment where team_board_idx = ? and id = ? order by reg_date desc");
											    		pstmt.setInt(1, idx);
											    		pstmt.setString(2, writer);
											    		rs = pstmt.executeQuery();
											    		...
											    		while(rs.next()){
															...
															<div class="comment">
															    ...
															    <form method="post" action="updateTeamReple.jsp?idx=<%=idx %>&commentIdx=<%=commentIdx %>">
															    <div class="comment-content">
															        <textarea class="form-control" rows="2" name="text" id="text" ><%=rs.getString("text") %></textarea>
															        <div style="float:right">
															            <button type="submit">수정</button>
															         	<button type="button" onclick="location.href='deleteTeamReple.jsp?idx=<%=idx%>&commentIdx=<%=commentIdx%>'">삭제</button>
															        </div>
															    </div>
															    </form>
															</div>
