...
<% String commentId = session.getAttribute("id")+"";
	...
	try {
		...
		String SQL = "SELECT * FROM team_board where id = ? and text = ?";
	...
	comment.setTeam_name(team_name);
	comment.setId(id);
...
<body>
	<%
		...
		int result = dao.teamBoardComment(comment.getIdx(), comment.getId(), comment.getCommentID(), comment.getText(), comment.getTeam_name());
